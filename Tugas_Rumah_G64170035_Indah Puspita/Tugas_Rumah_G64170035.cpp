...
static void scroll_callback(GLFWwindow* window, double x, double y)
{
    ...
    void pagarkiri(double x);
    void pagarkiridepan(double x);
    void pagarkanan(double x);
    void pagarkanandepan(double x);
    void pagardalamkiri();
    void rumah();
...
int main(void)
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        //pagar dalam kanan
        for(i=0;i<6;i++) pagarkanan(i);
        for(i=0;i<6;i++) pagarkanandepan(i);
        pintu();
        pagardalamkiri();
...
void background(){
    ...
    //rumput
    glBegin(GL_POLYGON);
    glColor3ub(132,178,80);
    glVertex2f(0, 369.43);
    glVertex2f(0, 800);
    glVertex2f(800, 800);
    glVertex2f(800, 369.43);
    glEnd();
}
...
void pagardalamkiri() {
    ...
    for(i=0;i<6;i++) pagarkiri(i);

    for(i=0;i<6;i++) pagarkiridepan(i);
